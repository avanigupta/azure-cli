...
from ._validators import (
    validate_appservice_name_or_id,
    validate_connection_string,
    validate_datetime,
    validate_export,
    validate_import,
    validate_import_depth,
    validate_query_fields,
    validate_feature_query_fields,
...
def load_arguments(self, _):
    ...
    fields_arg_type = CLIArgumentType(nargs='+',
                                      help='Customize output fields.',
                                      validator=validate_query_fields,
                                      arg_type=get_enum_type(['key',
                                                              'value',
                                                              'label',
                                                              'content_type',
                                                              'etag',
    ...
    with self.argument_context('appconfig kv import') as c:
        c.argument(
            'label',
            help="Imported KVs will be assigned with this label. If no label specified, will assign null label.")
        c.argument(
            'prefix',
            help="This prefix will be appended to the front of imported keys.")
        c.argument(
            'source',
            options_list=[
                '--source',
    ...
    with self.argument_context('appconfig kv import', arg_group='File') as c:
        ...
        c.argument(
            ...
            choices=[
                ...
                'yaml',
                'properties'],
            help='Imported file format. Required for file arguments.')
        c.argument(
            'depth',
            validator=validate_import_depth,
            help="Depth for flatterning the json or yaml file to key-value paris. Flattern to the deepest level by default.")
        c.argument(
            'separator',
            arg_type=get_enum_type(
                [
                    ...
                    '__',
                    '/']),
            help="Delimiter for flatterning the json or yaml file to key-value pairs. Required for importing hierarchical structure. Not applicable for property files.")
    ...
    with self.argument_context('appconfig kv import', arg_group='AppConfig') as c:
        ...
        c.argument(
            ...
            validator=validate_connection_string,
            help="Connections of access key and endpoint of the source store. ")
        c.argument(
            'src_key',
            help='If no key specified, import all keys by default. Support star sign as filters, for instance abc* means keys with abc as prefix. Similarly, *abc and *abc* are also supported.')
        c.argument(
            'src_label',
            help="Only keys with this label in source AppConfig will be imported. If no label specified, import keys with null label by default.")
    ...
    with self.argument_context('appconfig feature set') as c:
        ...
        c.argument(
            'label',
            help="If no label specified, set the feature flag with null label by default")
        c.argument(
            'description',
    ...
    with self.argument_context('appconfig feature filter add') as c:
        ...
        c.argument(
            'label',
            help="If no label specified, add to the feature flag with null label by default.")
        c.argument('filterName', help='Name of the filter to be added.')
        c.argument('filterParameters', arg_type=filter_parameters_arg_type)
        c.argument(
            'index',
            type=int,
            help='Zero-based index in the list of filters where you want to insert the new filter.')
    ...
    with self.argument_context('appconfig feature filter delete') as c:
        ...
        c.argument(
            'label',
            help="If no label specified, delete from the feature flag with null label by default.")
        c.argument('filterName', help='Name of the filter to be deleted.')
        c.argument(
            'index',
            type=int,
            help='Zero-based index of the filter to be deleted in case there are multiple instances with same filter name.')
    ...
    with self.argument_context('appconfig feature filter show') as c:
        ...
        c.argument(
            'label',
            help="If no label specified, show the feature flag with null label by default.")
        c.argument('filterName', help='Name of the filter to be displayed.')
        c.argument(
            'index',
            type=int,
            help='Zero-based index of the filter to be displayed in case there are multiple instances with same filter name.')
    ...
    with self.argument_context('appconfig feature filter list') as c:
        ...
        c.argument(
            'label',
            help="If no label specified, display filters from the feature flag with null label by default.")

    with self.argument_context('appconfig feature filter clear') as c:
        c.argument(
            'feature',
            help='Name of the feature whose filters need to be deleted.')
        c.argument(
            'label',
            help="If no label specified, clear filters from the feature flag with null label by default.")
